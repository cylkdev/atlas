defmodule AtlasProvider.Serializer do
  @moduledoc """
  ...
  terms via `AtlasProvider.Serializer.to_serial_term/2` (or a caller-supplied
  transformer).
  ...
  `AtlasProvider.Strings.string_to_atom/2`, which owns the atom-safety controls
  ...
  ## Responsibilities
    ...
    - Coerce non-JSON-able leaf values during serialization via a configurable
      transformer (defaults to `AtlasProvider.Serializer.to_serial_term/2`).
    - Apply caller-supplied value transforms during deserialization.
    - Forward atom-safety options to `AtlasProvider.Strings.string_to_atom/2`,
    ...
    - Read defaults for normalize/transform options from compile-time
      configuration under `config :mcp, :serializer, ...`, with
      per-call `opts` overriding them. Atom-safety defaults live in
      `AtlasProvider.Strings` (see its module docs).

  ## Options

    * `:to_case` -- target casing for keys. Defaults to `:camel` for
      `serialize/2` and `:snake` for `deserialize/2`. See `AtlasProvider.Casing` for
      the full list.
    * `:to_existing_atom`, `:strict`, `:allowed_keys` -- forwarded as-is to
      `AtlasProvider.Strings.string_to_atom/2`. See its docs for semantics and
      defaults.
    ...
    * `:to_serial_term` -- the value coercion strategy used by `serialize/2`
      ...
      function, or a `{module, function}` tuple. Defaults to `AtlasProvider.JSON`.

  ## Examples

      # Serialize a struct-free map to camelCase string keys
      AtlasProvider.Serializer.serialize(%{user_name: "ada", joined_at: ~D[2026-05-02]})
      #=> %{"userName" => "ada", "joinedAt" => "2026-05-02"}

      # Deserialize incoming camelCase JSON to existing snake_case atom keys
      AtlasProvider.Serializer.deserialize(
        %{"userName" => "ada"},
  ...
  #         `AtlasProvider.Strings.string_to_atom/2`, with leaves optionally
  ...
  #   `config :mcp, :serializer, ...` overrides.
  #   `default_options()` is their merge.
  #
  # Data Invariant:
  #   1. Per-call `opts` are merged on top of `default_options()` so
  ...
  #      `:allowed_keys` to `AtlasProvider.Strings.string_to_atom/2`
  #      unchanged.
  #
  ...
  ## Examples
      ...
      iex> AtlasProvider.Serializer.to_serial_term(~D[2026-05-02], [])
      "2026-05-02"

      iex> AtlasProvider.Serializer.to_serial_term(~T[10:20:30], [])
      "10:20:30"

      iex> AtlasProvider.Serializer.to_serial_term(42, [])
      42
  ...
  ## Parameters
    ...
    - `opts` - `keyword()`. Default `[]`.
        * `:to_case` - target casing for keys. Default `:camel`.
          Forwarded to `AtlasProvider.Casing.to_case/3`.
        ...
        * `:to_serial_term` - module, 2-arity function, or
          ...
          `AtlasProvider.Serializer` itself (i.e. `to_serial_term/2`).
        * `opts[:date][:format]`, `opts[:time][:format]`,
          `opts[:datetime][:format]` - forwarded to the date/time
          coercions in `to_serial_term/2`.

      `opts` is also forwarded to `AtlasProvider.Casing.to_case/3` so its
  ...
  `term()`. A tree mirroring the input shape with:
    ...
    - non-struct map keys converted via `Atom.to_string/1` (atoms),
      ...
      then recased through `AtlasProvider.Casing.to_case/3`.
  ...
  ## Raises
    ...
    - `ArgumentError` - if `opts[:to_serial_term]` is not `nil`, a
      ...
      tuple.
    - Any exception `AtlasProvider.Casing.to_case/3` raises (e.g. unknown
  ...
  ## Examples
      ...
      iex> AtlasProvider.Serializer.serialize(%{user_name: "ada"})
      %{"userName" => "ada"}

      iex> AtlasProvider.Serializer.serialize(%{joined_at: ~D[2026-05-02]})
      %{"joinedAt" => "2026-05-02"}

  """
  @spec serialize(term(), atom(), keyword()) :: term()
  def serialize(term, casing \\ :camel, opts \\ []) do
  ...
  `AtlasProvider.Casing.to_case/3`, and atomized via
  `AtlasProvider.Strings.string_to_atom/2`. Non-binary keys are returned
  ...
  ## Parameters
    ...
    - `opts` - `keyword()`. Default `[]`. Merged on top of
      `default_options()`. Recognised keys:
        ...
        * `:to_existing_atom`, `:strict`, `:allowed_keys` - forwarded
          to `AtlasProvider.Strings.string_to_atom/2`. See its docs for
  ...
  ## Raises
    ...
    - Any exception `AtlasProvider.Strings.string_to_atom/2` raises is
      ...
      `:allowed_keys`.
    - Any exception `AtlasProvider.Casing.to_case/3` raises is propagated.

  ## Examples

      iex> AtlasProvider.Serializer.deserialize(
      ...>   %{"userName" => "ada"},
      ...>   to_existing_atom: false,
      ...>   strict: true,
      ...>   allowed_keys: ["user_name"]
  ...
  """
  @spec deserialize(term(), atom(), keyword()) :: term()
  def deserialize(term, casing, opts \\ []) do
    opts = Keyword.merge(default_options(), opts)
    val_fun = opts[:transform_value]
    transform(term, fn key -> deserialize_key(key, casing, opts) end, val_fun)
  end

  # Transforms a single key. Binary keys are trimmed, stripped of `"`, optionally
  # normalized, snake-cased, and atomized via AtlasProvider.Strings.string_to_atom/2.
  # Non-binary keys are returned unchanged.
  defp deserialize_key(key, casing, opts) when is_binary(key) do
    case key |> trim_and_strip_quotes() |> normalize_key(opts) do
      string_key when is_binary(string_key) ->
        incoming_key = Casing.to_case(string_key, casing, opts)
  ...
  defp deserialize_key(key, _casing, _opts), do: key
  ...
  defp default_options do
    Keyword.merge(@default_options, Application.get_env(@app, :serializer) || [])
  end
end
